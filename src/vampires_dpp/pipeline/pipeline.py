...
class Pipeline:
    ...
    def run(self, filenames, num_proc: int | None = None, redo: str | None = None):
        ...
        ## force_adi/force_diff bypass the config flag so --redo always runs the stage.
        if (self.config.save_adi_cubes or force_adi) and redo not in ("diff", "pdi"):
            self.save_adi_cubes(force=force_adi)

        if (self.config.diff_images.make_diff or force_diff) and redo not in ("adi", "pdi"):
